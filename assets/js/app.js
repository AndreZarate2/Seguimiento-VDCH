...
const collapseSignatures=JSON.parse(localStorage.getItem('zc_collapse_signatures')||'{}');
const GANTT_ROW_H=56;
...
function saveCollapse(){localStorage.setItem('zc_collapsed',JSON.stringify(collapsed));localStorage.setItem('zc_collapse_signatures',JSON.stringify(collapseSignatures))}
function projectCollapseSignature(p){return tasksOf(p).filter(t=>hasChildren(p,t)).map(t=>String(t.outline||'')).join('|')+'::'+(p?.tasks?.length||0)}
function initializeCollapsedDefaults(){activeProjects().forEach(p=>{const sig=projectCollapseSignature(p); if(collapseSignatures[p.id]!==sig){tasksOf(p).filter(t=>hasChildren(p,t)).forEach(t=>{collapsed[p.id+'|'+t.outline]=true}); collapseSignatures[p.id]=sig;}}); saveCollapse()}
function nearestVisibleAncestor(task,visibleByOutline){if(!task)return null; if(visibleByOutline.has(String(task.outline)))return visibleByOutline.get(String(task.outline)); const parents=parentOutlines(task.outline).reverse(); for(const o of parents){if(visibleByOutline.has(String(o)))return visibleByOutline.get(String(o));} return null}
...
function renderGantt(pid){const p=projectById(pid), el=qs('gantt-'+pid); if(!el)return; const rows=filteredTasks(pid); const dated=rows.filter(t=>taskStartMs(p,t)&&taskFinishMs(p,t)); const min=dated.length?Math.min(...dated.map(t=>taskStartMs(p,t))):Date.now(); const max=dated.length?Math.max(...dated.map(t=>taskFinishMs(p,t))):Date.now()+86400000; const total=Math.max(1,max-min); const todayPct=Math.max(0,Math.min(100,((day(CONTROL_DATE)-min)/total)*100));
  const rowHTML=rows.map((t,i)=>{const s=taskStartMs(p,t), f=taskFinishMs(p,t); const left=s?((s-min)/total)*100:0; const width=(s&&f)?Math.max(.3,((f-s)/total)*100):0; const st=statusOf(t); const pad=Math.max(0,Number(t.outline_level||1)-1)*12; const hc=hasChildren(p,t); const col=collapsed[pid+'|'+t.outline]; return `<div class="gantt-row ${isSummary(t)?'group':''} ${col?'is-collapsed':''}" data-uid="${esc(t.uid)}" data-index="${i}"><div class="gantt-label" style="padding-left:${pad}px">${hc?`<button class="tree-toggle gantt-toggle ${col?'collapsed':''}" title="${col?'Expandir':'Contraer'}" onclick="toggleNode('${pid}','${esc(t.outline)}')">${col?'+':'−'}</button>`:'<span class="tree-toggle-spacer"></span>'}<span class="outline">${esc(t.outline)}</span><span class="gantt-label-text">${esc(t.name)}</span></div><div class="gantt-track"><i class="gantt-bar ${isSummary(t)?'summary':progressClass(st)}" style="left:${left}%;width:${width}%"></i></div></div>`}).join('');
  const h=Math.max(1,rows.length*GANTT_ROW_H);
  el.innerHTML=`<div class="gantt-inner"><div class="gantt-tree-wrap"><div class="gantt-scale"><div>Actividad</div><div>${fmt(min)} - ${fmt(max)}</div></div><div class="gantt-legend"><span class="gantt-legend-item"><span class="gantt-legend-line"></span>Amarre / predecesor</span><span class="gantt-legend-item"><span class="gantt-legend-line blocked"></span>Predecesor pendiente</span></div><div class="gantt-chart-area"><div class="today-line" style="left:calc(var(--label-w) + var(--gap) + ${todayPct}%);"></div>${rowHTML}<svg class="gantt-dep-svg" viewBox="0 0 100 ${h}" preserveAspectRatio="none">${dependencyArrowSVG(pid,rows,min,total)}</svg></div></div></div>`}
function taskDescendants(p,t){const o=String(t.outline||''); return tasksOf(p).filter(x=>x.uid!==t.uid && String(x.outline||'').startsWith(o+'.'))}
function taskStartMs(p,t){const own=d(t.start); if(own)return own.getTime(); const vals=taskDescendants(p,t).map(x=>d(x.start)).filter(Boolean).map(x=>x.getTime()); return vals.length?Math.min(...vals):null}
function taskFinishMs(p,t){const own=d(t.finish); if(own)return own.getTime(); const vals=taskDescendants(p,t).map(x=>d(x.finish)).filter(Boolean).map(x=>x.getTime()); return vals.length?Math.max(...vals):null}
function dependencyArrowSVG(pid,rows,min,total){const p=projectById(pid); if(!p||!rows.length)return ''; const visibleByOutline=new Map(rows.map((r,i)=>[String(r.outline),{task:r,i}])); const rowByUid=new Map(rows.map((r,i)=>[String(r.uid),{task:r,i}])); const edges=new Map();
  tasksOf(p).forEach(t=>{predecessorLinks(t).forEach(l=>{if(!l.task)return; const from=nearestVisibleAncestor(l.task,visibleByOutline); const to=nearestVisibleAncestor(t,visibleByOutline); if(!from||!to||from.task.uid===to.task.uid)return; const key=from.task.uid+'>'+to.task.uid+'>'+(l.type||'FS'); if(!edges.has(key))edges.set(key,{from,to,type:l.type||'FS',lag:l.lag||'',blocked:statusOf(l.task)!=='Terminado',source:l.task,target:t});})});
  let out=''; edges.forEach(e=>{const f=taskFinishMs(p,e.from.task), s=taskStartMs(p,e.to.task); if(!f||!s)return; const x1=Math.max(0,Math.min(100,((f-min)/total)*100)); const x2=Math.max(0,Math.min(100,((s-min)/total)*100)); const y1=e.from.i*GANTT_ROW_H+(GANTT_ROW_H/2), y2=e.to.i*GANTT_ROW_H+(GANTT_ROW_H/2); const mid=Math.max(1,Math.min(99,(x1+x2)/2)); const cls=e.blocked?'blocked':''; const fill=e.blocked?'#b45309':'#475569'; const title=`${e.type}${e.lag?' '+e.lag:''}: ${e.from.task.outline} → ${e.to.task.outline}`; out+=`<path class="gantt-dep-path ${cls}" d="M ${x1} ${y1} C ${mid} ${y1}, ${mid} ${y2}, ${x2} ${y2}"><title>${esc(title)}</title></path><circle class="gantt-dep-dot ${cls}" cx="${x1}" cy="${y1}" r=".45"/><polygon class="gantt-dep-head ${cls}" points="${x2},${y2} ${x2-1.25},${y2-2.4} ${x2-1.25},${y2+2.4}" fill="${fill}" opacity=".82"/>`}); return out}
...
window.addEventListener('DOMContentLoaded',async()=>{await loadData(); initializeCollapsedDefaults(); applyDateSettings(); setTimeout(()=>showOpeningAlerts(false),600)});
